...
sock_connect(struct sock_addr * const * sas)
{
	...
	/* Connnect. */
	if ((s = sock_connect_blocking(sas)) == -1)
		goto err0;
...
/**
 * sock_connect_blocking(sas):
 * Behave as sock_connect, except that the socket is not marked as
 * non-blocking.
 */
int
sock_connect_blocking(struct sock_addr * const * sas)
{
	int s = -1;
